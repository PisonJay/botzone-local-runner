...
struct subproc_t {
    ...
    void Read(bool is_bot=false,char *buffer=buf,size_t len=BUFFER_SIZE) {
        // read to buf
        ...
        if(is_bot) {
            ...
            while(!flag) {
                ...
                if(check_end(buf+i)) flag=true;
                if(!flag) pos+=read(fd[0],buffer+(last_pos=pos),len-pos);
            }
        } else {
            while((pos+=read(fd[0],buffer+(last_pos=pos),len-pos))!=last_pos);
        }
        buffer[pos]=0;
    }
    void Close() {
...
int main(int argc,const char* argv[]) {
    ...
    while(true) {
        ...
        judger.Close();judger.Read();reader.parse(buf,val);
